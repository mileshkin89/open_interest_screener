...
class BinanceListener(BaseExchangeListener):
    ...
    async def fetch_usdt_symbols(self) -> list[str]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=10) as resp:
                    if resp.status != 200:
                        ...
                        print(f"[ERROR] Failed to fetch symbols: {resp.status}, {text}")
                        return []
            ...
            print(f"[ERROR] Network error fetching USDT symbols: {e}")
        except Exception as e:
            print(f"[ERROR] Unexpected error fetching USDT symbols: {e}")
        ...
        try:
            async with session.get(url, params=params, timeout=10) as resp:
                ...
                if not isinstance(data, list):
                    print(f"[ERROR] OI data not list for {symbol}: {data}")
                    return []
    ...
    async def fetch_ohlcv(self, symbol: str, start_date: int, end_date: int,
        ...
        try:
            async with session.get(url, params=params, timeout=10) as resp:
                ...
                data = await resp.json()
                if not isinstance(data, list):
                    print(f"[ERROR] OHLCV data not list for {symbol}: {data}")
                    return []
